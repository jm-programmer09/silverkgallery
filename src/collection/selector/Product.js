...
export default function Product () {
  ...
  let display_base_dimensions = "Call us for available sizes";
    ...
    <>
      <section className="products">
              ...
              <div className="hierarchyrow">
                ...
                { subid === undefined && (
                  <>
                    ...
                    <Link to={`/collection/${categories}`}>{categories.replaceAll("-", " ")}</Link>
                  </>
                )}
                ...
                <Link style={subid === undefined ? { color: "rgb(130, 130, 130)"} : {}} to={`/collection/${categories}/${themes}`}>{themes.replaceAll("-", " ")}</Link>
                {
                  subid !== undefined && (
                    <>
                      <svg xmlns="http://www.w3.org/2000/svg" width="15px" height="11px" viewBox="0 0 24 24" fill="none">
                      ...
                      <Link style={{ color: "rgb(130, 130, 130)"}} to={`/collection/${categories}/${themes}/?sub${themes}=${id}`}>{id.replaceAll("-", " ")}</Link>
                    </>
                  )
                }
          ...
          <div className="right">
            ...
            {product.author ? 
              <h4 style={{ color: "black", marginBottom: "20px"}}> <span>{product.type} by </span>{product.author.replaceAll(/^(.)|\s+(.)/g, c => c.toUpperCase()).replaceAll("-", " ")}</h4>
              :
              <>
                {categories.toLowerCase() === "animation" && <h4 style={{ color: "black", marginBottom: "20px"}}><span>by </span>{themes.replaceAll(/^(.)|\s+(.)/g, c => c.toUpperCase()).replaceAll("-", " ")}</h4>}
              </>
            }
